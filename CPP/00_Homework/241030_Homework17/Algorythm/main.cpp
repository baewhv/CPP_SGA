...
public:
	...
	bool operator<(const Player& other) const //멤버 변수를 수정하지 못하도록.
	{
...
int main()
{
	...
	// std::sort; quick sort를 C++스타일로 iterator가 동작하도록 제작.
	// std::partial_sort(); heap sort
	...
	for (int i = 0; i < 10; i++)
	{
		Player* p = new Player("hi" + to_string(rand() % 100), rand()%100);

		players.push_back(p);
	}


	struct PlayerSortFunc
	{
	...
	std::sort(players.begin(), players.end(), PlayerSortFunc());
	
	//string -> char형을 담는 vector가 부모인 자식.

	v.clear(); //기존 데이터를 그대로 두고 size만 0으로 표기.

	//vector<int>().swap(v); //rvalue의 특징을 이용한 제거.
